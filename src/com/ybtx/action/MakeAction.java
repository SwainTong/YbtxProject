...
public class MakeAction extends ActionSupport implements ModelDriven<Make>{
	...
	//根据id查询生产记录  更新用
	public String fingByIdForUpdate() {
		MakePage makePage = makeService.findById(make.getMakeId(),currentPage, pageSize);
		ServletActionContext.getRequest().setAttribute("makePage", makePage.getDataList().get(0));
		return "update";		
	}
	...
	public String searchEmployeeWageMonthly(){
		String sql = "SELECT employee.employeeId,SUM(makeAmount * productWage) 工资 " + 
				"FROM make,product,employee " + 
				"WHERE make.makeProduct = product.productId " + 
				"	AND make.makeEmployee = employee.employeeId " + 
				"	AND MONTH(make.makeDate) = '" + make.getMonth() +
        ...
        {    
            System.out.println(obj[0]+" -- "+ obj[1]);
            Employee employee = employeeService.findById((int)obj[0]);
            obj[0] = employee;
            totalWage = totalWage + (double)obj[1];
        }
        ServletActionContext.getRequest().setAttribute("EmployeeWageList", list);
        ServletActionContext.getRequest().setAttribute("totalWage",totalWage);
        ServletActionContext.getRequest().getSession().setAttribute("Month",make.getMonth());
		return "EmployeeWageList";
	}
	//按天统计某个工人的工资
	public String employeeWageByDay() {
		//HttpServletRequest request = ((ServletRequestAttributes)RequestContextHolder.getRequestAttributes()).getRequest();  
		int month = (int) ServletActionContext.getRequest().getSession().getAttribute("Month");
		//int month = 7;//(int) request.getAttribute("Month");
		String sql = "SELECT SUM(make.makeAmount * productWage),DAY(make.makeDate) day " + 
				"FROM make,product,employee " + 
				"WHERE make.makeProduct = product.productId " + 
				"	AND make.makeEmployee = employee.employeeId " + 
				"	AND employee.employeeId = " + make.getEmployeeId()+
				"	AND MONTH(make.makeDate)=  " + month +
